...
uint256 hashGenesisBlock("0xac1ddcdb9578f7786808cca67f3f317661aca89043258b589996ebd8dc2b9b9c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Neutralitycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Neutralitycoin Signed Message:\n";
...
{
    // Neutralitycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 220 * COIN; //220 coins per block

    // Subsidy is cut in half every 259200 blocks, which will occur approximately every 6 months
    nSubsidy >>= (nHeight / 259200); // Neutralitycoin: 259200 blocks in ~6 months
    
    if(nHeight > 777600) // no blocks after 18 months
        nSubsidy = 0;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Neutralitycoin: 1 day diff readjust
static const int64 nTargetSpacing = 1 * 60; // Neutralitycoin: 1 minute block time
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Neutralitycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Neutralitycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Neutralitycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x72be0b03fe1db61eab884c63becf5eb4c5279cd9415ab3d6f546ae0b41ddb635");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 19/Feb/2014 Minimum Wage Increase Would Have Mixed Effects, C.B.O. Report Says";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392828872;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1085711401;

        if (fTestNet)
        {
            block.nTime    = 1392818369;
            block.nNonce   = 185538563;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc6eb31cc0c2f74c7728167a37539862a14e55e5a3e3a963f8f35a394ef39693a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Neutralitycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NeutralitycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NeutralitycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NeutralitycoinMiner(CWallet *pwallet)
{
    printf("NeutralitycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("neutralitycoin-miner");
    ...
    try { loop {
        ...
        printf("Running NeutralitycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NeutralitycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NeutralitycoinMiner, pwallet));
}
